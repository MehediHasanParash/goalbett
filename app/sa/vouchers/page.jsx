...
export default function SubAgentVouchersPage() {
  ...
  const fetchVouchers = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/subagent/vouchers?${params.toString()}`, {
  ...
  const handleCreateVouchers = async () => {
    try {
      ...
      const res = await fetch("/api/subagent/vouchers", {
        method: "POST",
        headers: {
  ...
  const handleCancelVoucher = async (voucherId) => {
    ...
    try {
      const token = getAuthToken()
      const res = await fetch(`/api/subagent/vouchers/${voucherId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const navItems = [
    { icon: TrendingUp, label: "Dashboard", id: "dashboard", path: "/sa/dashboard" },
    { icon: Users, label: "My Players", id: "players", path: "/sa/dashboard" },
    { icon: Ticket, label: "Vouchers", id: "vouchers", path: "/sa/vouchers" },
    { icon: Wallet, label: "Wallet", id: "wallet", path: "/sa/dashboard" },
  ]
  ...
  return (
    <div className="min-h-screen bg-[#0A1A2F]">
      ...
      <div className={`lg:hidden fixed inset-0 z-40 ${sidebarOpen ? "block" : "hidden"}`}>
        ...
        <div className="absolute left-0 top-0 h-screen w-64 bg-[#0D1F35] border-r border-[#2A3F55] p-6">
          ...
          <div className="space-y-4">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => {
                  if (item.path !== "/sa/vouchers") {
                    router.push(item.path)
                  }
                  setSidebarOpen(false)
                }}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${
                  item.id === "vouchers" ? "bg-[#FFD700] text-[#0A1A2F]" : "text-[#B8C5D6] hover:bg-[#1A2F45]"
                }`}
              >
      ...
      <div className="lg:ml-64 p-4 md:p-8">
        <div className="max-w-7xl mx-auto">
          {/* Header */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 pl-12 lg:pl-0">
            <div>
