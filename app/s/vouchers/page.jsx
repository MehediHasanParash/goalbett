...
export default function SuperAdminVouchersPage() {
  ...
  const fetchVouchers = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/super/vouchers?${params.toString()}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      const data = await res.json()
  ...
  const handleCreateVouchers = async () => {
    try {
      ...
      const res = await fetch("/api/super/vouchers", {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        ...
        setCreateForm({ amount: 10, quantity: 1, prefix: "VCH", expiryDays: 30, description: "", tenantId: "" })
        fetchVouchers()
  ...
  const handleCancelVoucher = async (voucherId) => {
    ...
    try {
      const token = getAuthToken()
      const res = await fetch(`/api/super/vouchers/${voucherId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  return (
    <SuperAdminLayout title="Voucher Management" description="Generate and manage prepaid vouchers across all tenants">
      <div className="space-y-6">
        {/* Success/Error Messages */}
        {success && (
          <div className="p-4 rounded-lg bg-green-500/20 border border-green-500/30 text-green-400">{success}</div>
        )}
        {error && <div className="p-4 rounded-lg bg-red-500/20 border border-red-500/30 text-red-400">{error}</div>}
                ...
                <table className="w-full">
                  <thead className="bg-[#0A1A2F]">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-[#F5F5F5]/60 uppercase">Tenant</th>
                      ...
                      <th className="px-4 py-3 text-right text-xs font-medium text-[#F5F5F5]/60 uppercase">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#FFD700]/10">
                    {filteredVouchers.map((voucher) => (
                      <tr key={voucher._id} className="hover:bg-[#FFD700]/5">
                        ...
                        <td className="px-4 py-4 text-sm text-[#F5F5F5]/70">{voucher.tenantId?.name || "N/A"}</td>
                        <td className="px-4 py-4">
                        ...
                        <td className="px-4 py-4 text-sm text-[#F5F5F5]/70">
                          {voucher.redeemedBy ? (
                            <div>
                              <p className="text-white">{voucher.redeemedBy.fullName || voucher.redeemedBy.username}</p>
        ...
        <DialogContent className="bg-[#0D2137] border-[#FFD700]/20 text-white">
          <DialogHeader>
            ...
            <DialogDescription className="text-[#F5F5F5]/70">
              Create prepaid vouchers for players to redeem
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-4">
            <div>
              <label className="text-sm font-medium text-[#F5F5F5]/70">Select Tenant</label>
              <Select value={createForm.tenantId} onValueChange={(v) => setCreateForm({ ...createForm, tenantId: v })}>
                <SelectTrigger className="mt-1 bg-[#0A1A2F] border-[#FFD700]/20">
            ...
            <div className="p-4 rounded-lg bg-[#FFD700]/10 border border-[#FFD700]/20">
              <div className="flex justify-between items-center">
                <span className="text-[#F5F5F5]/70">Total Value:</span>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={creating || createForm.amount < 1 || createForm.quantity < 1 || !createForm.tenantId}
              className="bg-[#FFD700] hover:bg-[#FFD700]/90 text-[#0A1A2F]"
            >
              {creating ? (
                <>
