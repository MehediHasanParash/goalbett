...
export class SettlementService {
  static async generateAgentCommissionSettlement({ tenantId, agentId, periodStart, periodEnd, preparedBy }) {
    ...
    try {
      ...
      const commissionRate = config.commissions?.agentCommission ? config.commissions.agentCommission / 100 : 0.05
      const grossAmount = ggr * commissionRate

      const operatorDeductions = await JurisdictionRulesService.calculateOperatorDeductions({
        tenantId,
        ggr,
        turnover: totalStake,
        netProfit: ggr,
      ...
      const deductions = {
        platformFee: grossAmount * ((config.commissions?.platformFee || 0) / 100),
        taxes: 0,
        chargebacks: 0,
        adjustments: 0,
        jurisdictionDeductions: [],
      }

      if (operatorDeductions.deductions.length > 0) {
        for (const ded of operatorDeductions.deductions) {
          deductions.jurisdictionDeductions.push({
            name: ded.name,
            label: ded.label,
          ...
          deductions.taxes += ded.amount
        }
      }

      const totalDeductionAmount = deductions.platformFee + deductions.taxes + deductions.chargebacks + deductions.adjustments
      const netAmount = Math.max(0, grossAmount - totalDeductionAmount)
        ...
        [
          {
            ...
            metadata: {
              ...
              totalPayout,
              ggr,
              commissionRate: config.commissions?.agentCommission || 5,
              jurisdictionRuleId: operatorDeductions.ruleId,
              jurisdictionRuleVersion: operatorDeductions.ruleVersion,
              jurisdictionCountryCode: operatorDeductions.countryCode,
              operatorTaxTotal: operatorDeductions.totalDeducted,
            },
          },
        ],
        { session },
  ...
  static async calculateGGR({ tenantId, startDate, endDate }) {
    ...
    const operatorDeductions = await JurisdictionRulesService.calculateOperatorDeductions({
      tenantId,
      ggr: totalGGR,
      turnover: sportsTotalStake + casinoTotalStake,
      netProfit: totalGGR,
    ...
    return {
      ...
      startDate,
      endDate,
      jurisdictionDeductions: operatorDeductions.deductions,
      totalOperatorTax: operatorDeductions.totalDeducted,
      netGGR: operatorDeductions.netRevenue,
      jurisdictionRuleId: operatorDeductions.ruleId,
      jurisdictionRuleVersion: operatorDeductions.ruleVersion,
      jurisdictionCountryCode: operatorDeductions.countryCode,
    }
  ...
  static async calculatePlayerTaxSummary({ tenantId, startDate, endDate }) {
    const bets = await Bet.find({
      tenantId,
      status: "won",
      settledAt: { $gte: startDate, $lte: endDate },
      "taxDetails.deductions": { $exists: true, $ne: [] },
    })

    const summary = {
      totalBetsWithTax: bets.length,
      totalGrossWin: 0,
      totalTaxDeducted: 0,
      totalNetWin: 0,
      deductionsByType: {},
    ...
    for (const bet of bets) {
      if (bet.taxDetails) {
        summary.totalGrossWin += bet.taxDetails.grossWin || 0
        summary.totalTaxDeducted += bet.taxDetails.totalDeductions || 0
        summary.totalNetWin += bet.taxDetails.netWin || 0

        if (bet.taxDetails.deductions) {
          for (const ded of bet.taxDetails.deductions) {
            if (!summary.deductionsByType[ded.name]) {
              summary.deductionsByType[ded.name] = {
                name: ded.name,
                count: 0,
                totalAmount: 0,
                percentage: ded.percentage,
            ...
            summary.deductionsByType[ded.name].count += 1
            summary.deductionsByType[ded.name].totalAmount += ded.amount
          }
        }
      }
    }
