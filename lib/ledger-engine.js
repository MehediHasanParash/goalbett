...
export class LedgerEngine {
  ...
  /**
   * Record tax/charity deduction
   * CRITICAL: This creates audit trail for every deduction
   */
  static async recordTaxDeduction({
    playerWalletId,
    userId,
    tenantId,
    systemAccountId,
    ...
    const transactionTypeMap = {
      win_tax: "WIN_TAX_DEDUCTION",
      betting_tax: "BETTING_TAX_DEDUCTION",
      withholding_tax: "WITHHOLDING_TAX_DEDUCTION",
      vat: "VAT_DEDUCTION",
      excise_duty: "EXCISE_DUTY_DEDUCTION",
      charity: "CHARITY_DEDUCTION",
      social_responsibility: "SOCIAL_RESPONSIBILITY_DEDUCTION",
      gaming_levy: "GAMING_LEVY_DEDUCTION",
      other: "TAX_DEDUCTION",
    }

    const transactionType = transactionTypeMap[deductionDetails.name] || "TAX_DEDUCTION"
    ...
    return this.createEntry({
      ...
      creditAccount: {
        accountType: "system",
        accountName: deductionDetails.destinationAccount || "tax_payable",
      },
      amount,
      currency,
      transactionType,
      referenceType: "bet",
      referenceId: betId,
      description: `${deductionDetails.name} deduction - ${deductionDetails.percentage}% of ${deductionDetails.calculationBase}`,
      createdBy,
      metadata: {
        betId,
        systemAccountId,
  ...
  /**
   * Record bet winning with automatic tax/charity deductions
   * This replaces the old recordBetWinning and applies jurisdiction rules
   */
  static async recordBetWinningWithDeductions({
    walletId,
    userId,
    tenantId,
    betId,
    ...
    try {
      // 1. Credit gross winnings to player
      const winningEntry = await this.createEntry({
        tenantId,
        debitAccount: {
          accountType: "revenue",
          accountName: "Bet Payouts",
        ...
        referenceType: "bet",
        referenceId: betId,
        description: `Bet winning - ${betId} (Stake: ${stakeAmount}, Gross Win: ${grossWin})`,
        createdBy,
        metadata: {
          stakeAmount,
          grossWin,
          netWin,
          totalDeductions: grossWin - netWin,
          jurisdictionRuleId,
        },
      })

      // 2. Apply each deduction (tax, charity, etc.)
      const deductionEntries = []
      for (const deduction of deductions) {
        const deductionEntry = await this.recordTaxDeduction({
          playerWalletId: walletId,
          userId,
          tenantId,
          betId,
