...
export class SettlementEngine {
  ...
  static async settleBet(bet, event, settledBy, session) {
    ...
    if (betStatus !== "pending") {
      // CRITICAL: Apply jurisdiction rules if bet won
      let deductionSummary = null
      let netPayout = payout

      if (betStatus === "won" && payout > 0) {
        // Get tenant and jurisdiction rules
        const tenant = await Tenant.findById(bet.tenantId).session(session)
        const countryCode = tenant?.metadata?.country || "US"

        // Get active jurisdiction rule
        const jurisdictionRule = await JurisdictionRule.getActiveRule(countryCode)

        if (jurisdictionRule) {
          // Calculate deductions
          const grossWin = payout - bet.stake
          deductionSummary = jurisdictionRule.calculatePlayerDeductions(grossWin, bet.stake, payout)

          // Net payout after deductions
          netPayout = bet.stake + deductionSummary.netAmount

          // Store deduction details in bet
          bet.taxDetails = {
            jurisdictionRuleId: jurisdictionRule._id,
            ruleVersion: jurisdictionRule.version,
            countryCode: jurisdictionRule.countryCode,
            grossWin,
            totalDeductions: deductionSummary.totalDeducted,
            netWin: deductionSummary.netAmount,
            deductions: deductionSummary.deductions,
          }
        }
      }
      ...
      // Create ledger entry for winning/void WITH DEDUCTIONS
      if (payout > 0) {
        const wallet = await Wallet.findOne({ userId: bet.userId }).session(session)
        if (wallet) {
          if (deductionSummary && deductionSummary.deductions.length > 0) {
            // Use enhanced ledger method that applies deductions
            await LedgerEngine.recordBetWinningWithDeductions({
              walletId: wallet._id,
              userId: bet.userId,
              tenantId: bet.tenantId,
              betId: bet._id,
              stakeAmount: bet.stake,
              grossWin: payout - bet.stake,
              deductions: deductionSummary.deductions,
              netWin: deductionSummary.netAmount,
              currency: bet.currency,
              jurisdictionRuleId: bet.taxDetails?.jurisdictionRuleId,
              createdBy: settledBy,
            })
          } else {
            // No deductions, use old method
